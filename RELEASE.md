...
Keep the root `vendor/` directory available to `.distignore`: the stable deploy workflow installs production Composer dependencies in the checkout before `10up/action-wordpress-plugin-deploy` filters that checkout. The local builder uses its own `rsync` exclusion for `vendor/`, then reconstructs a clean production-only `vendor/` inside staging. Package generation fails early if `.distignore` would remove all Composer runtime files from the stable deploy path.

...
- Working base: `origin/develop` at `10750b6`.
...
- Prepared repository assets already present on `develop`:
  ...
  - `.wordpress-org/screenshot-1.png` - CleanLinks list table
  - `.wordpress-org/screenshot-2.png` - Edit Link redirection settings
  - `.wordpress-org/screenshot-3.png` - Import screen
  - `.wordpress-org/screenshot-4.png` - Export screen
...
- Include the screenshot files and matching captions listed below.
...
| `.wordpress-org/screenshot-3.png` | Import screen showing the supported import flow. | Import supported links from the CleanLinks import screen. |
| `.wordpress-org/screenshot-4.png` | Import/Export screen showing the CSV export action. | Export CleanLinks data to CSV from the Import/Export screen. |
...
- Treat `npm run plugin-zip` as code package validation only; it does not prove WordPress.org asset deployment because the `.wordpress-org` files are handled by the WordPress.org asset path.
